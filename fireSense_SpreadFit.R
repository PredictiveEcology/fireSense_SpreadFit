  ...
  parameters = rbind(
    ...
    defineParameter(name = "cacheId_DE", class = "character", default = NULL,
                    desc = paste("An optional character string representing a `cacheId` to recover from the Cache.",
                                 "Using `reproducible >= 2.0.10.9016`, this can be set to 'previous',",
                                 "meaning the `Cache` call will get the previous item in the cache",
                                 "that matches `P(sim)$rep` (see that param).")),
    ...
    defineParameter(name = "doObjFunAssertions", class = "logical", default = TRUE,
                    desc = paste("This is passed to `objFunSpreadProb`:",
                                 "`TRUE` will do some diagnostics but is slower;",
                                 "`FALSE` for operational runs."),
    defineParameter(name = "initialpop", class = "numeric", default = NULL,
                    desc = paste("A numeric matrix of dimensions `NCOL = length(lower)` and `NROW = NP`.",
                                 "This will be passed into `DEoptim` through",
                                 "`control$initialpop = P(sim)$initialpop` if it is not NULL")),
    defineParameter(name = "iterDEoptim", class = "integer", default = 500L,
    ...
    defineParameter(name = "libPathDEoptim", class = "character", default = .libPaths()[1],
    ...
    defineParameter(name = "mode", class = "character", default = "fit",
                    desc = paste("One or more of 'debug', 'fit', 'visualize'.",
                                 "'debug' will trigger running of the objective function with visuals;",
                                 "'fit' will trigger `DEoptim`;",
                                 "'visualize' will trigger visualization after `DEoptim.`",
                                 "For 'visualize', DE object must be findable, either in `sim`,",
                                 "on disk or a cloud URL (specified with `urlDEOptimObject`).")),
    defineParameter(name = "mutuallyExclusiveCols", "list", list("youngAge" = c("class", "nonForest")), NA, NA,
                    desc = paste("a named list of mutually exclusive covariates;",
                                 "see `?fireSenseUtils::makeMutuallyExclusive`.")),
    ...
    defineParameter(name = "objfunFireReps", class = "integer", default = 100L,
    ...
    defineParameter(name = "rep", class = "integer", 1L, NA, NA,
                    desc = paste("An optional integer indicating which replicate run this represents.",
                                 ...
                                 "Cache will think that the second run should recover the cache result,",
                                 "unless this `rep` is modified"),
