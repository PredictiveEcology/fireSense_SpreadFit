...
`fireAttributes`|`"fireAttributesFireSense_SpreadFit"`|a character vector indicating the name of an object of class `SpatialPointsDataFrame` describing fires starting locations, final sizes ('size' column), and possibly the starting dates ('date' column) if fires are to be spread at different time intervals. If the 'date' column is not present, all fires are assumed to have started at the same time interval.
...
`nIterDEoptim`|`500`|integer defining the maximum number of iterations allowed (DEoptim optimizer).
...
inputs <- data.frame(
  objectName = c("fireAttributesFireSense_SpreadFit", "beta", "theta"),
...
tweakSimList <- function(sim) {
  loadFiles(sim)
  sim[["fireAttributesFireSense_SpreadFit"]][["date"]] <- NULL
