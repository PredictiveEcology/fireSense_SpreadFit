...
# Data dependencies
## Input data
An object of class `RasterLayer` or `RasterStack` or named `lists` of `RasterLayers` or `RasterStacks`, in which to look for variables with which to predict. `RasterStacks` are useful in cases where fires didn't start the same year (for example), and thus should not be spread at the same time but are used to describe a single fire size distribution.
